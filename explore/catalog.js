export const visualizations = [
  ...
  },
  {
    ...
    instructions:
      "Evolution Scatter Plot uses the existing generation control and hover behavior. Distribution Histogram lets you choose All time or a generation, switch between births and distinct-name counts, choose 1%, 2%, 4%, or 10% male-share bins, and use a linear or logarithmic y-axis; hover a bar for its five most popular names in that period. In the Table view, search for a name, set a minimum all-time-birth threshold, or move the Table order slider from All Female to All Male.",
    methodology:
      "The Evolution Scatter Plot compares male and female birth frequencies for selected prominent unisex names on logarithmic axes. In the Distribution Histogram, each name is assigned to a bin using the male share of its recorded births within the selected period. The y-axis can show either the total births represented by names in each bin or the number of distinct names. All time uses the full national record; generation choices use the listed generation year ranges. Bin labels show boundaries: for example, 48–52% male means [48, 52), so 48% is included and 52% is not; the final bin includes 100%. Colors move from female-leaning pink through light purple near an even split to male-leaning blue. The Table remains an all-time summary.",
    dataThrough: 2025,
    iframeHeight: 850,
    embedMode: "scale",
    embedWidth: 1100,
